...
/*      throw new Exception("Problem reading default properties: "
 ...
 public static /*@pure@*/ int maxIndex2(double [] doubles) {

    double maximum = Double.MIN_VALUE;
    int maxIndex = 0;

    for (int i = 0; i < doubles.length; i++) {
      if ((!Double.isNaN(doubles[i])) && (doubles[i] > maximum)) {
	maxIndex = i;
	maximum = doubles[i];
      }
    }
  ...
  public static int max(double [] doubles)
  {
      ...
      {
        //  if (doubles[i]>max){
        if (((doubles[i]>max) && (!Double.isNaN(doubles[i]))) || (Double.isNaN(max))){
            max=doubles[i];
            index=i;
          }
      }
  ...
  public static /*@pure@*/ int minIndex2(double [] doubles) {

    double minimum = Double.MAX_VALUE;
    int minIndex = 0;

    for (int i = 0; i < doubles.length; i++) {
      if ((!Double.isNaN(doubles[i])) && (doubles[i] < minimum)) {
	minIndex = i;
	minimum = doubles[i];
      }
    }
