...
public class Stats implements Serializable {
    ...
    public double calculaStdDevM(double m, Instances ins, int index) {
        double sigma=Double.NaN;
        double sumatoria = 0,countador=0;
        for (int i = 0; i < ins.numInstances(); i++) {
            double val=ins.instance(i).value(index);
            if (!Double.isNaN(val))
            {
                sumatoria += Math.pow((val - m), 2);
                countador++;
            }
        }
        sigma = Math.sqrt((1 / (countador-1)) * sumatoria);
        return sigma;
    }

    public double calculaStdDevM(double m, Instances ins, int index,int from, int to) {
        double sigma=Double.NaN;
        double sumatoria = 0,countador=0;
    ...
    public double calculaMediana(Instances ins, int index) {
        double mediana = Double.NaN;      
        double attVals[];
                
        ArrayList a=new ArrayList();
        for(int i=0;i<ins.numInstances();i++)       
            if (!ins.instance(i).isMissing(index))
                a.add(ins.instance(i).value(index));

        if (a.size()>0)
        {
        attVals=new double[a.size()];
        for(int i=0;i<a.size();i++)
            attVals[i]=(double)Double.valueOf(a.get(i).toString());


        int midPoint = attVals.length / 2;


        if ((attVals.length % 2) == 0) {
            mediana = (Utils.kthSmallestValue(attVals, midPoint) + Utils.kthSmallestValue(attVals, midPoint + 1)) / 2;            
        } else {
            mediana = Utils.kthSmallestValue(attVals, midPoint + 1);
        }
    ...
    public double calculaMediana(Instances ins, int index, int from, int to) {
        double mediana = Double.NaN;
        double attVals[];
    ...
    /*
    public double calculaMediana2(final Instances ins, int idx) {
        double mediana = Double.NaN;
        ins.sort(idx);
        int midPoint = ins.numInstances() / 2;
        //Es par
        if ((ins.numInstances() % 2) == 0) {
            mediana = (ins.instance(midPoint - 1).value(idx) + ins.instance(midPoint).value(idx)) / 2;
        } else {
            mediana = ins.instance(midPoint).value(idx);
        }
        return mediana;
    }
 * 
