...
import java.util.*;
import com.github.aonagarcia.gaussianmixturemodel.*;
...
public class Discretize extends PotentialClassIgnorer
    ...
    protected boolean m_UseEqualFrequency = true;
    /** The default columns to discretize */
    protected String m_DefaultCols;
    protected Instances m_validationModel;
    private boolean m_UseMixtureGaussians = false;
    private boolean m_UseEqualWidthBinning = false;
    ...
    protected void calculateCutPoints() {
        ...
        for (int i = getInputFormat().numAttributes() - 1; i >= 0; i--) {
            if ((m_DiscretizeCols.isInRange(i))
                ...
                if (m_FindNumBins) {
                    findNumBins(i);
                } else if (m_UseEqualWidthBinning) {
                    calculateCutPointsByEqualWidthBinning(i);
                } else if (m_UseEqualFrequency) {
                    calculateCutPointsByEqualFrequencyBinning(i);
                } else if (this.m_UseMixtureGaussians) {
                    calculateCutPointsByGMM(i);
                }
            }
        }
    }

    public void setUseEqualWidthBinning(boolean b) {
        m_UseEqualWidthBinning = b;
    }

    public void setValidationMOdel(Instances inst) {
        m_validationModel = inst;
    }
    ...
    protected void calculateCutPointsByEqualWidthBinning(int index) {
        ...
        double max = Double.NaN, min = Double.NaN, currentVal;

        if (m_validationModel != null) {
            int idx2 = m_validationModel.getValueIndex(getInputFormat().attribute(index).name(), 0);
            min = m_validationModel.instance(idx2).value(m_validationModel.attribute("SPminTol"));
            max = m_validationModel.instance(idx2).value(m_validationModel.attribute("SPmaxTol"));
        }
        ...
        if ((m_NumBins > 1) && (binWidth > 0)) {
            ...
            cutPoints[m_NumBins - 1] = max;
            cutPoints[m_NumBins] = min;
    ...
    /**
     * Obtiene el valor máximo y mínimo
     *
     * @param index el índice del atributo del que se obtendrán los valores
     */
    protected double[] getMinMax(int index) {
    ...
    protected void calculateCutPointsByEqualFrequencyBinning(int index) {
        ...
        if (cpindex == 0) {
            double[] m_vec = getMinMax(index); //Cuando hay �nicamente un valor para todas las instancias la funci�n getMinMax regresa el mismo valor
            m_CutPoints[index] = m_vec;     //en las dos elementos del arreglo, por lo tanto m_CutPoints[index] tendra dos valores [min,max]

        } else {
            double[] cp = new double[cpindex + 2]; // se crea un arreglo de cpindex elementos + 2 que ser�n los valores min y max
            for (int i = 0; i < cpindex; i++) {
                cp[i] = cutPoints[i];
            }

            //Se agrego esto para incluir los extremos de los datos min y max en lugar de (-inf) e (inf)
            double[] m_vec = getMinMax(index);
            cp[cpindex] = m_vec[0];
            cp[cpindex + 1] = m_vec[1];
            //
    ...
    protected void findNumBins(int index) {
        ...
        if (m_validationModel != null) {
            ...
            //Esto se hace por si alguno de los valores min/max es NaN entonces se busca el valor
            //min o max de los datos. Se puede optimizar si se asigna directamente de la estructura
            //m_validationModelo que ya lo tiene almacenado
            if ((Double.isNaN(min)) || (Double.isNaN(max))) {
                // Find minimum and maximum
        ...
        if ((bestNumBins > 1) && (binWidth > 0)) {
            cutPoints = new double[(bestNumBins - 1) + 2]; //se crean (bestNumBins - 1) elementos + 2 para los extremos
            for (int i = 1; i < bestNumBins; i++) {
                cutPoints[i - 1] = min + binWidth * i;
            }
            //Se agregan  los extremos de los datos min y max en lugar de (-inf) e (inf)
            double[] m_vec = getMinMax(index);
            cutPoints[bestNumBins - 1] = m_vec[0];
            cutPoints[bestNumBins] = m_vec[1];
        } else if ((bestNumBins == 1) && (binWidth >= 0)) {
            cutPoints = new double[2];
            //<-- NEW cutPoints[0]=max;
            cutPoints[0] = max;
            cutPoints[1] = min;
            ...
            if ((m_DiscretizeCols.isInRange(i))
                ...
                if (!m_MakeBinary) {
                    ...
                    if (m_CutPoints[i] == null) {
                        //-->
                        attribValues.addElement("N/A");
                    } else {
                        /*Si s�lo hay dos puntos definidos para los intervalos*/
                        //if ((m_CutPoints[i].length==2) && (m_CutPoints[i][0]==m_CutPoints[i][1])) {
                        String e1 = Utils.doubleToString(m_CutPoints[i][0], 6);
                        String e2 = Utils.doubleToString(m_CutPoints[i][1], 6);
                        if ((m_CutPoints[i].length == 2) && (e1.equalsIgnoreCase(e2))) {
                            attribValues.addElement(Utils.doubleToString(m_CutPoints[i][0], 6));
                        } else {
                            for (int j = 0; j <= m_CutPoints[i].length - 2; j++) {
                                if (j == 0) {
                                    attribValues.addElement(Utils.doubleToString(m_CutPoints[i][m_CutPoints[i].length - 1], 6) + "_"
                                            + Utils.doubleToString(m_CutPoints[i][j], 6));
                                } else if (j == m_CutPoints[i].length - 2) {
                                    attribValues.addElement(Utils.doubleToString(m_CutPoints[i][j - 1], 6) + "_"
                                            + Utils.doubleToString(m_CutPoints[i][m_CutPoints[i].length - 2], 6));
                                } else {
                                    attribValues.addElement(Utils.doubleToString(m_CutPoints[i][j - 1], 6) + "_"
                                            + Utils.doubleToString(m_CutPoints[i][j], 6));
                                }
    ...
    public void setUseMixtureGaussians(boolean b) {
        m_UseMixtureGaussians = b;
    }

    public void calculateCutPointsByGMM(int index) {
        double max = Double.NaN, min = Double.NaN, currentVal;
        if (m_validationModel != null) {
            int idx2 = m_validationModel.getValueIndex(getInputFormat().attribute(index).name(), 0);
            min = m_validationModel.instance(idx2).value(m_validationModel.attribute("SPminTol"));
       ...
       /* MixtureGaussians GMM = new MixtureGaussians(this.m_NumBins);
        m_CutPoints[index] = GMM.algorithmEM(getInputFormat(), index, min, max);
        GMM = null;
         */
        try {            
            Clusterer clusterer=new EM();
            ((EM)clusterer).setNumClusters(m_NumBins);
            Instances _inst=getInputFormat();
            Instances attToProcess=new Instances(_inst,_inst.attribute(index).name());
            clusterer.buildClusterer(attToProcess);
            m_CutPoints[index] =clusterer.getCutPoints(min, max);            
            clusterer=null;
            attToProcess=null;
        } catch (Exception ex) {
            ex.printStackTrace();            
