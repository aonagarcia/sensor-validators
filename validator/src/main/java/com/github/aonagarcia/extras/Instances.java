...
public class Instances implements Serializable {
    ...
    protected int totalInstWithMtto=0;
    ...
    /**
  * JHV - 12/04/2012
  *Creates a set of instances containing just the values of the
  * indicated sigle attribute
  *
  */
  public Instances(Instances ins, String attName)
  {
    FastVector atts;
    ...
    m_ClassIndex = -1;
    m_Attributes = atts;
    m_Instances = new FastVector(ins.numInstances());
    for (int i = 0; i < ins.numInstances(); i++) {
      double[] val = new double[1];
      val[0]=ins.instance(i).value(ins.attribute(attName).index());
      this.add(new Instance(1.0, val));
    }
  }
    ...
    public void add(/*@non_null@*/Instance instance) {
        ...
        newInstance.setOriginalIdx(m_Instances.size());
    ...
    public void deleteAttributeAt(int position) {
        ...
        if (position == m_ClassIndex) {
            throw new IllegalArgumentException("No se puede eliminar la clase");
        }
        freshAttributeInfo();
    ...
    /**
     * Regresa un arreglo con los valores distintos de atributos numericos.
     *
     * @param attIndex el atributo
     * @return Arreglo que contiene los valores(double) distintos.
     */
    public Object[] getDistinctValues(int attIndex) {

        if (attribute(attIndex).isNumeric()) {
            Object distintValues[] = new Object[numDistinctValues(attIndex)];
            double[] attVals = attributeToDoubleArray(attIndex);
            int[] sorted = Utils.sort(attVals);
            double prev = 0;
            int counter = 0;
    ...
    public /*@pure@*/ Object[] getDistinctValues(/*@non_null@*/Attribute att) {

        return getDistinctValues(att.index());
    }
        ...
        while (Attribute.ARFF_ATTRIBUTE.equalsIgnoreCase(tokenizer.sval)) {
            ...
            if (tokenizer.ttype == StreamTokenizer.TT_WORD) {
                ...
                if (tokenizer.sval.equalsIgnoreCase(Attribute.ARFF_ATTRIBUTE_REAL)
                    ...
                    m_Attributes.addElement(new Attribute(attributeName, (FastVector) null,
                            numAttributes()));
                    readTillEOL(tokenizer);
                ...
                m_Attributes.addElement(new Attribute(attributeName, attributeValues,
                        numAttributes()));
            }
            getLastToken(tokenizer, false);
            getFirstToken(tokenizer);
    ...
    public int numMissingData() {
        int nMissing = 0;
        for (int row = 0; row < this.numInstances(); row++) {
            for (int var = 0; var < this.numAttributes(); var++) {
                if (instance(row).getErrorType(var) == 1) {
                    nMissing++;
                }
            }
        }
    ...
    public int numMissingData(String varName) {
        int nMissing = 0;
        int varIdx = this.attribute(varName).index();
        for (int row = 0; row < this.numInstances(); row++) {
            if (instance(row).getErrorType(varIdx) == 1) {
                nMissing++;
            }
        }
        return nMissing;
    ...
    public int numOutOfRangeData() {
        int nOutOfRange = 0;
        for (int row = 0; row < this.numInstances(); row++) {
            for (int var = 0; var < this.numAttributes(); var++) {
                if (instance(row).getErrorType(var) == 2) {
                    nOutOfRange++;
                }
            }
        }
    ...
    public int numOutOfRangeData(String varName) {
        int nOutOfRange = 0;
        int varIdx = this.attribute(varName).index();
        for (int row = 0; row < this.numInstances(); row++) {
            if (instance(row).getErrorType(varIdx) == 2) {
                nOutOfRange++;
            }
        }
        return nOutOfRange;
    ...
    public int numAbruptChanges() {
        int nAbruptChanges = 0;
        for (int row = 0; row < this.numInstances(); row++) {
            for (int var = 0; var < this.numAttributes(); var++) {
                if (instance(row).getErrorType(var) == 3) {
                    nAbruptChanges++;
                }
            }
        }
        return nAbruptChanges;
    }

    public int numAbruptChanges(String varName) {
        int nAbruptChanges = 0;
        int varIdx = this.attribute(varName).index();
        for (int row = 0; row < this.numInstances(); row++) {
            if (instance(row).getErrorType(varIdx) == 3) {
                nAbruptChanges++;
            }
    ...
    public int numRelatedVariablesErrors() {
        int nRelatedVariablesErrors = 0;
        for (int row = 0; row < this.numInstances(); row++) {
            for (int var = 0; var < this.numAttributes(); var++) {
                if (instance(row).getErrorType(var) == 4) {
                    nRelatedVariablesErrors++;
                }
            }
        }
        return nRelatedVariablesErrors;
    }

    public int numRelatedVariablesErrors(String varName) {
        int nRelatedVariablesErrors = 0;
        int varIdx = this.attribute(varName).index();
        for (int row = 0; row < this.numInstances(); row++) {
            if (instance(row).getErrorType(varIdx) == 4) {
                nRelatedVariablesErrors++;
            }
        }
        return nRelatedVariablesErrors;
    ...
    public boolean[] localOutLiers(int attIdx)
    {
        boolean[] localOutliers=new boolean[this.numInstances()];
        for(int i=0; i< this.numInstances(); i++)
        {
            if(this.instance(i).getErrorType(attIdx)==5)
            {
                localOutliers[i]=true;
            }
        }
        return localOutliers;
    }

    public boolean isEmpty(int idx) {
        for (int row = 0; row < this.numInstances(); row++) {
            if (instance(row).isMissing(idx) == false) {
                return false;
            }
        }
    ...
    public void restoreAttribute(Instances backUpInst, Attribute att) {
        int currentPos = att.index();
        deleteAttributeAt(currentPos);
        insertAttributeAt(backUpInst.attribute(att.name()), currentPos);
        for (int row = 0; row < numInstances(); row++) {
            int InstanceIdx = instance(row).getOriginalIdx();
            int idxBckUp = backUpInst.attribute(att.name()).index();
            instance(InstanceIdx).setValue(att, backUpInst.instance(InstanceIdx).value(idxBckUp));
            attribute(att.name()).setDiscretizedFlag(false,-1,-1);
        }
    }

    /**
     * Regresa el índice de la instancia que contiene el valor
     * value en el atributo attIdx (este método es utilizado solo
     * para el objeto validationModel con la finalidad de buscar
     * las variables definidas las cuales aparecen como filas en
     * el objeto instances en al atributo 0.
     *
     * @param value El valor a buscar
     *@return el ínidce de la instancia de la primera ocurrencia de value
     *@return -1 si no se encuentra el valor
     */
    public int getValueIndex(String value, int attIdx) {
        for (int i = 0; i < this.numInstances(); i++) {
            String temp = instance(i).stringValue(attIdx);
    ...
    /**
     * M�todo para normalizar los datos
     * @param s Escala
     * @param t Traslaci�n
     *
     * newVal=(((val-min)/(max-min))*s)+t
     */

    public double[] normalizaAtt(int attIdx, double s,double t) {

            double[] minmax=new double[2];
            double max = Double.MIN_VALUE;
            double min = Double.MAX_VALUE;
            if (this.attribute(attIdx).isNumeric()) {
                for (int i = 0; i < this.numInstances(); i++) {
                    double val = this.instance(i).value(attIdx);
                    if (val > max) {
                        max = val;
                    }
                    if (val < min) {
                ...
                for (int i = 0; i < this.numInstances(); i++) {
                    double val = this.instance(i).value(attIdx);
                    double newVal=(((val-min)/(max-min))*s)+t;
                    //double newVal=val/max;
                    this.instance(i).setValue(attIdx, newVal);
                }
            }
    ...
    /**
     * M�todo que regresa un arreglo de Objetos con los elementos del
     * atributo especificado como par�metro
     * @param �ndice del atributo del cual se quiere obtener el arreglo de elememtos.
     */
    public Object[] attToArray(int attIdx) {
        Object[] data = new Object[this.numInstances()];
        for (int k = 0; k < this.numInstances(); k++) {
            if (this.instance(k).isMissing(attIdx)) {
                data[k] = (Object) null;
            } else if (this.attribute(attIdx).isNominal()) {
                data[k] = (Object) this.instance(k).stringValue(attIdx);
            } else if (this.attribute(attIdx).isNumeric()) {
                data[k] = (Object) this.instance(k).value(attIdx);
            }
        }
        return data;
     ...
     public void setInstancesWithMttos(boolean[] s)
    {
        for(int i=0; i<numInstances();i++)
        {
            //if (s[i]==true)
                //totalInstWithMtto++;
            this.instance(i).setM_HasMtto(s[i]);
        }
    }

     public int[] getInstancesIdxWithMttos()
    {
        int numMttos=0;
        for(int i=0; i<numInstances();i++)
            numMttos=instance(i).isM_HasMtto()?numMttos+1:numMttos;

        int[] idxs=new int[numMttos];
        int cont=0;
        for(int i=0; i<numInstances();i++)
        {
            if(instance(i).isM_HasMtto())
            {
                idxs[cont]=i;
                cont++;
            }
