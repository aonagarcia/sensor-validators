...
public class Instance implements Copyable, Serializable {
    ...
    private ArrayList<Double> _pweight = new ArrayList<Double>();
    
    protected  int[] m_AttErrorType;
    
    protected  double[] m_AttProbableValue;
    
    protected  boolean[] m_AttChangeValue;
    
    protected  short[] m_metodoUtilizado;
    
    protected boolean m_deleted=false;
    
    protected int originalIdx;

    protected  boolean m_HasMtto=false;
    ...
    protected Instance(/*@non_null@*/ Instance instance) {
        ...
        m_AttErrorType=new int [m_AttValues.length];
        m_AttProbableValue=new double [m_AttValues.length];
        m_AttChangeValue=new boolean [m_AttValues.length];
        m_metodoUtilizado=new short [m_AttValues.length];
        
        for (int i = 0; i < m_AttValues.length; i++) {
            m_AttErrorType[i]=0;
            m_AttProbableValue[i]=Double.NaN;
        }
    ...
    public void setErrorType(int attIndex, int errorType) {
        
        m_AttErrorType[attIndex]=errorType;        
    }
    
    public int getErrorType(Attribute att) {
        
        return getErrorType(att.index());
    }
    
    public int getErrorType(int attIndex) {
        
        return m_AttErrorType[attIndex];
    }
    
    public void setProbableValue(int attIndex, double value) {
        
        m_AttProbableValue[attIndex]=value;        
    }
    
    /**Si se establece 1 el metodo utilizado es Instanciando la CM
     si es 2 entonces es la MODA, para datos normales el valor es 0*/
     public void setMetodoUtilizado(int attIndex, short b) {        
        m_metodoUtilizado[attIndex]=b;        
    }
    
    public short getMetodoUtilizado(int attIndex) {        
        return m_metodoUtilizado[attIndex];
    }
     
    public double getProbableValue(int attIndex) {
        
        return m_AttProbableValue[attIndex];
    }
    
    
    public boolean changeValue(int attIndex) {
        
        return m_AttChangeValue[attIndex];
    }    
    
    public void setChangeValue(int attIndex, boolean b) {        
        m_AttChangeValue[attIndex]=b;        
    }
    /**
     * Sets a specific value in the instance to the given value
     * (internal floating-point format). Performs a deep copy
    ...
    public double getWeight(int index) {
        return _pweight.get(index);
    }
    
    public void setWeight(int index,double w) {
        _pweight.set(index, w);
    }

    public boolean isM_HasMtto() {
        return m_HasMtto;
    }

    public void setM_HasMtto(boolean m_HasMtto) {
        this.m_HasMtto = m_HasMtto;
    }
