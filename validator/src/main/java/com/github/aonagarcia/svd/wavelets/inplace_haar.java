...
package com.github.aonagarcia.svd.wavelets;
...
<b>5</b>  1  0 -2 -3  1 -1  0
  ...
  public void wavelet_calc( double[] values ) {
	...
	for (int j = 0; j < len; j++) {
	  ...
	  if (c_ix < len) {
          double a=0,c=0;
             /* if (Double.isNaN(values[a_ix]) && !Double.isNaN(values[c_ix]))
              {
                  a=values[c_ix]/2;
                  c=-1.0*(values[c_ix]/2);
              }else
              ...
              if (Double.isNaN(values[a_ix]) && Double.isNaN(values[c_ix]))
              {
                ...
                a = ((values[a_ix] + values[c_ix]) / Math.sqrt(2));
                c = ((values[a_ix] - values[c_ix]) / Math.sqrt(2));
              }
	    values[a_ix] = a;
	    values[c_ix] = c;
	  }
