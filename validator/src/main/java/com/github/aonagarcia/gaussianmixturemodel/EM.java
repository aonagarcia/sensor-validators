...
public class EM
    ...
    public double[] getCutPoints(double min, double max) {
        double[] cutPoints = new double[m_num_clusters + 1];
        double[] means = new double[m_num_clusters];
        double[] sigmas = new double[m_num_clusters];
        for (int i = 0; i < m_num_clusters; i++) {
            means[i] = this.m_modelNormal[i][0][0];
            sigmas[i] = this.m_modelNormal[i][0][1];
        }

        int[] orderedMeans = Utils.stableSort(means);

        /*for (int i=0; i<m_num_clusters;i++)
        {
        System.out.println("Mean: "+means[orderedMeans[i]]+" sigmas: "+sigmas[orderedMeans[i]]);
        }
        System.out.println();
         *
         */
        for (int i = 0; i < m_num_clusters - 1; i++) {
            cutPoints[i] = getGaussianIntersectionPoint(min, max, means[orderedMeans[i]], means[orderedMeans[i + 1]], sigmas[orderedMeans[i]], sigmas[orderedMeans[i + 1]]);
        }
        cutPoints[m_num_clusters - 1] = max;
        cutPoints[m_num_clusters] = min;
    ...
    public double getGaussianIntersectionPoint(double min, double max, double m0, double m1, double s0, double s1) {
        double a = 0, b = 0, c = 0;
